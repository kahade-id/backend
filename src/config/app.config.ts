...
/**
 * (non-numeric KAHADE_FEE_RATE) fails loudly instead of silently producing NaN →
 * BigInt(0) → 0% fee on all orders.
 * arithmetic without any floating-point conversion.
 */
...
function parseFeeRate(envVar: string, fallback: number): number {
  const raw = process.env[envVar];
  if (raw === undefined || raw === '') return fallback;
  const value = parseFloat(raw);
  if (isNaN(value) || value < 0 || value > 10) {
    throw new Error(
      `CONFIG ERROR: ${envVar}="${raw}" is not a valid fee rate. ` +
...
function strictInt(envVar: string, fallback: number, min: number, max: number): number {
  const raw = process.env[envVar];
...
function parseFeeRateBps(envVar: string, fallback: number): number {
  const raw = process.env[envVar];
  if (raw === undefined || raw === '') return fallback;
  const value = parseInt(raw, 10);
  if (isNaN(value) || value < 0 || value > 1000) {
    throw new Error(
      `CONFIG ERROR: ${envVar}="${raw}" is not a valid fee rate in basis points. ` +
...
export const appConfig = registerAs('app', () => ({
  ...
  orderMinValue: (() => { const v = parseInt(process.env.ORDER_MIN_VALUE || '10000', 10); if (isNaN(v) || v < 0) throw new Error(`CONFIG ERROR: ORDER_MIN_VALUE="${process.env.ORDER_MIN_VALUE}" is not a valid positive integer`); return v; })(),
  orderMaxValue: (() => {
    const v = parseInt(process.env.ORDER_MAX_VALUE || '1000000000', 10);
    if (isNaN(v) || v < 0) throw new Error(`CONFIG ERROR: ORDER_MAX_VALUE="${process.env.ORDER_MAX_VALUE}" is not a valid positive integer`);
    const minVal = parseInt(process.env.ORDER_MIN_VALUE || '10000', 10);
    if (!isNaN(minVal) && v < minVal) throw new Error(`CONFIG ERROR: ORDER_MAX_VALUE (${v}) must be >= ORDER_MIN_VALUE (${minVal})`);
    return v;
  })(),
  ...
  // without a code change. Fee amounts for virtual accounts and e-wallets.
  ...
  // SECURITY: orphan cleanup defaults to DRY-RUN. The current orphan-detection
  // logic relies solely on a 24h Redis confirmation key — it has NO database
  // cross-check, so any consumed file (dispute evidence, KYC photo, delivery
  // proof, avatar, etc.) whose `confirmed_upload:` key has been deleted on
  // consume or has expired naturally would be incorrectly classified as
  // orphan and permanently deleted from R2/S3. Set ORPHAN_CLEANUP_ENABLED=true
  // ONLY after a per-table DB reference check is added to cleanupBucket().
