...
export class DeliveryProofService {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt === MAX_RETRIES) {
          this.logger.error(
            `${label} gave up after ${MAX_RETRIES} attempts`,
  ...
  private runPostCommitBestEffort(task: () => Promise<void> | void, label: string): void {
    void Promise.resolve()
      ...
      .catch((error: unknown) => {
        this.logger.warn(
          `${label} post-commit side effect failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      });
  }
  ...
  private validateFileKeys(keys: string[], userId: string): void {
    if (!Array.isArray(keys) || keys.length > 10) {
      throw new BadRequestException({
        ...
        message: 'A maximum of 10 delivery proof files is allowed',
      });
    }
    ...
    for (const key of keys) {
      if (typeof key !== 'string' || seen.has(key) || !key.startsWith(DELIVERY_PROOF_KEY_PREFIX)) {
        throw new BadRequestException({
          code: ErrorCodes.VALIDATION_ERROR,
          message: 'File key must be a unique delivery proof upload key',
        });
      }
      seen.add(key);
      ...
      if (
        ...
        throw new BadRequestException({
          code: ErrorCodes.VALIDATION_ERROR,
          message: 'Invalid file key or file does not belong to user',
        });
      }
    }
  }
  ...
  async submitProof(
    orderId: string,
    userId: string,
    dto: { description: string; fileUrls?: string[]; linkUrls?: string[] },
  ): Promise<object> {
    const description = typeof dto.description === 'string' ? dto.description.trim() : '';
    if (description.length < 10 || description.length > 2000) {
      throw new BadRequestException({
        code: ErrorCodes.VALIDATION_ERROR,
        message: 'Delivery proof description must be between 10 and 2000 characters',
      });
    }
    ...
    if (
      linkUrls.length > 5 ||
      linkUrls.some(
        url => typeof url !== 'string' || url.length > 1000 || !/^https?:\/\//i.test(url),
      )
    ) {
      throw new BadRequestException({
        code: ErrorCodes.VALIDATION_ERROR,
        message: 'Delivery proof links must be HTTP(S) URLs and no more than 5 links',
      });
    }
          ...
          async tx => {
            ...
            if (lockedOrder.sellerId !== userId)
              throw new ForbiddenException({
                ...
                message: 'Only seller can submit delivery proof',
              });
            if (
              lockedOrder.status !== OrderStatus.PROCESSING &&
              lockedOrder.status !== OrderStatus.IN_DELIVERY
            ) {
              throw new BadRequestException({
                code: ErrorCodes.INVALID_ORDER_STATUS,
                message: 'Order must be in PROCESSING or IN_DELIVERY status',
              });
            }
            if (
              lockedOrder.orderType === OrderType.PHYSICAL_GOODS &&
              (!lockedOrder.trackingNumber || !lockedOrder.courierName)
            ) {
              throw new BadRequestException({
                code: ErrorCodes.VALIDATION_ERROR,
                message:
                  'Tracking number and courier are required before submitting physical delivery proof',
              });
            }
            ...
            if (existing)
              throw new BadRequestException({
                code: ErrorCodes.DELIVERY_PROOF_ALREADY_EXISTS,
                message: 'Delivery proof already submitted and pending review',
              });
            ...
            if (lockedOrder.status === OrderStatus.PROCESSING) {
              ...
              if (orderUpdated.count === 0) {
                throw new BadRequestException({
                  code: ErrorCodes.INVALID_ORDER_STATUS,
                  message: 'Order status has already changed',
                });
              }
              await tx.orderStatusHistory.create({
                data: {
                  ...
                  reason: 'Delivery proof submitted',
                },
              });
            }
            ...
            return {
              ...
              orderTitle: lockedOrder.title,
              orderPublicId: lockedOrder.orderId,
            };
          },
            ...
            (p.fileUrls as string[]).map(key =>
              key.startsWith('uploads/')
                ? this.uploadService.generateDownloadUrl(key, 3600).catch(err => {
                    this.logger.warn(
                      `Failed to sign delivery proof key ${key}: ${err instanceof Error ? err.message : String(err)}`,
                    );
                    return null;
                  })
                : Promise.resolve(key),
  ...
  async confirmDelivery(
    ...
    const proof = await this.prisma.deliveryProof.findFirst({
      where: proofId
        ? { id: proofId, orderId: order.id, status: 'SUBMITTED' }
        : { orderId: order.id, status: 'SUBMITTED' },
    });
    if (!proof)
      throw new NotFoundException({
        code: ErrorCodes.DELIVERY_PROOF_NOT_FOUND,
        message: 'No pending delivery proof found',
      });
  ...
  async rejectDelivery(
    ...
    note: string,
    proofId?: string,
  ): Promise<{ message: string; escalatedToDispute?: boolean }> {
    const normalizedNote = typeof note === 'string' ? note.trim() : '';
    if (normalizedNote.length < 10 || normalizedNote.length > 1000) {
      throw new BadRequestException({
        code: ErrorCodes.VALIDATION_ERROR,
        message: 'Rejection note must be between 10 and 1000 characters',
      });
    }
    const order = await this.prisma.order.findFirst({ where: { orderId, deletedAt: null } }); // AUDIT-16
    if (!order)
    ...
    if (order.buyerId !== userId)
      throw new ForbiddenException({
        code: ErrorCodes.NOT_ORDER_PARTICIPANT,
        message: 'Only buyer can reject delivery',
      });
    if (order.status !== OrderStatus.IN_DELIVERY) {
      throw new BadRequestException({
        code: ErrorCodes.INVALID_ORDER_STATUS,
    ...
    const nextDisputeId = async (): Promise<string> => {
      if (escalationDisputeId === null) {
        escalationDisputeId = generateDisputeId(
          await this.serialService.getNextForPrefix('dispute_serial'),
        );
      }
        ...
        this.prisma.$transaction(
          async tx => {
            ...
            if (
              !lockedOrder ||
              lockedOrder.status !== OrderStatus.IN_DELIVERY ||
              lockedOrder.buyerId !== userId
            ) {
              throw new BadRequestException({
                code: ErrorCodes.INVALID_ORDER_STATUS,
                message: 'Order is no longer in delivery and cannot be rejected',
              });
            }
            ...
            if (!lockedProof || lockedProof.status !== 'SUBMITTED') {
              throw new BadRequestException({
                code: ErrorCodes.DELIVERY_PROOF_NOT_FOUND,
                message: 'Delivery proof has already been reviewed',
              });
            }
            ...
            newRejectionTotal = await tx.deliveryProof.count({
    ...
    if (shouldEscalate) {
      this.logger.log(
        `Order ${orderId} auto-escalated to dispute after ${newRejectionTotal} delivery proof rejections`,
      );
